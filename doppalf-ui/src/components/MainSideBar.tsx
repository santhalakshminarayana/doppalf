...
type Props = {
    ...
    setOpenInfoBar: Dispatch<SetStateAction<boolean>>,
}

export default function MainSideBar({ isInfoBarOpen, setOpenInfoBar }: Props) {
    const onClickIcon = (name: string) => {
        setOpenInfoBar(!isInfoBarOpen)
    }
    ...
    return (
        <div className={`flex flex-col items-center p-3 ${isInfoBarOpen? "null": "border-solid border-r border-tomato-800/40 dark:border-dark_mint_cream-800/20"}`}>
            <button 
                ...
                className="flex flex-col justify-center items-center my-2 w-20 h-20 hover:bg-sidebar_icon_bg/20 hover:rounded-lg dark:hover:bg-dark_sidebar_icon_bg/5"
            >
                ...
                <p className='font-tilt-neon font-semi-bold text-md text-tomato dark:text-dark_mint_cream/75'>Settings</p>
            </button>
